...
namespace DagEdit
{
    internal sealed class SourceConnector : Connector
    {
        ...
        protected override void HandlePointerReleased(object? sender, PointerReleasedEventArgs args)
        {
            ...
            var parent = this.GetParentVisualByName<Canvas>("PART_ItemsHost");
            var currentPosition = parent != null ? args.GetPosition(parent) : default;
            var elementUnderPointer =
                parent?.GetClosestControlUnderPointer<Connector>(currentPosition);
        ...
        protected override void RaiseConnectionCompletedEvent(
            Connector? connector,
            Point? inAnchor,
            Guid? inNodeId,
            Point? outAnchor,
            Guid? outNodeId)
        {
            var args = new PendingConnectionEventArgs(
